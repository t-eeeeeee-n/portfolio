...
`:root` の素のトークン = **paper（暖色ベース）**。デプロイ既定は body 側で `data-theme="dark"` を当てるので、ベースは黒系（`tmp/portfolio/app.jsx` の `TWEAK_DEFAULTS.theme = "dark"` と一致）。`body[data-theme="..."]` で `paper` / `light` / `dark` を切替可能。

### 2.1 Zone トークン（重要：名前と色が一致しない）

| 役割 | Zone 名（クラス）| dark テーマ（既定） | paper テーマ | light テーマ |
|------|-----------|--------|-----|-----|
| ベース面 | `.zone-dark` | 真っ黒 `#08080a` | 暖色系の薄グレー `#ece9e0` | 純白系 `#fafafa` |
| 強調面 | `.zone-light` | 黒 `#0e0e12` | 純白 `#ffffff` | 純白 `#ffffff` |

既定の dark テーマでは「`.zone-dark` = 黒」「`.zone-light` = 黒系の少し明るい層」で名前と実色がだいたい一致する。**`paper` / `light` に切り替えた途端に `.zone-dark` が dark じゃなくなる**点に注意。tmp/portfolio 全域で使われており `--d-*` 変数群と一貫しているので、MVP ではリネームしない（リネームするなら全プロジェクトで一斉に）。
...
ダーク時は `.nav` 背景・`.contact-cta` グラデ・`.pipeline` シャドウなどが個別に上書きされる（`styles.css` の `body[data-theme="dark"] {...}` ブロック参照）。これらは既定で常に適用される。
