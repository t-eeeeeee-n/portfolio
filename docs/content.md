...
| 項目 | 値 |
|------|-----|
| サイト名 | teeen.lab — Engineering Lab / Product Studio |
| ハンドル | `teeen`（履歴書の `t-eeeeeee-n` とメール `t.eeee.n` から派生） |
| 本名 | 新井 天翔 / Tensho Arai |
| ロゴマーク | `t.n`（白文字 / オレンジグラデ角丸 / mono フォント） |
| 言語 | 日本語（`<html lang="ja">`） |
| Base | Tokyo · JST |
| 公開 URL | `https://teeen.vercel.app`（取得不可なら `teeen-arai.vercel.app`） |
...
```
[t.n] teeen.lab   Projects   Lab   Notes   About   Career   Contact   ↗ Lab
```
...
`放送局基幹システム更新` `帳票電子化システム` `自社ホームページ` `予約システム`
...
```
© 2026 teeen · Tokyo, JST   Built with Next.js · React · honest effort.       last commit · 2026.05.10    ● online
```
