...
export const careerSummary =
  'Web と AI を軸に、企画・要件定義から設計・実装・GCP での運用まで一人で完走できるフルスタックエンジニアです（エンジニア歴約 6 年）。直近はアイタイズで、メディア業界向けの業務オペレーション支援 AI エージェント基盤 PoC をリードとして推進するなど、生成 AI / AI Agent のプロダクト実装に注力しています。個人開発でも AI 設計支援 SaaS「SpecPilot」、スーパー価格比較サービス「ヤスイミセ」を、企画から運用まで一貫して開発。0 → 1 の立ち上げと、生成 AI を「実際に使われる・運用に耐える機能」へ落とし込む実装を得意としています。';

/* 活かせる経験・スキル — the 3 pillars to skim before the detail. */
export const strengths: ResumeStrength[] = [
  {
    title: '0 → 1 を一人で完走する実装力',
    body: '企画・MVP スコープ策定から UI / API / DB / GCP インフラ / CI/CD / 監視まで、プロダクトの全工程を一人で設計・実装・運用できます。「ヤスイミセ」「SpecPilot」は個人で全工程を担当しました。',
  },
  {
...
export const selfPr: string[] = [
  'フルスタックで Web サービス・SaaS を開発しています。最近は特に AI を活用したプロダクト開発や AI Agent 周りに注力しており、生成 AI を「デモで終わらせず、運用に乗る機能」へ落とし込むことに関心があります（個人開発・業務の両方で継続的に手を動かしています）。',
